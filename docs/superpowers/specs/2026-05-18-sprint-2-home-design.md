...
├── ContactCtaBanner.tsx            ← server (CTA-баннер под Услугами)
├── FAQ.tsx                         ← server обёртка
├── FAQAccordion.tsx                ← client (Radix Accordion)
└── ContactForm.tsx                 ← server (UI-каркас без логики)

components/ui/
├── Badge.tsx                       ← НОВЫЙ примитив (используется в ServiceCard)
└── Checkbox.tsx                    ← НОВЫЙ примитив (обёртка над @radix-ui/react-checkbox для ContactForm)
...
**Что НЕ трогаем:** Header, Footer, MobileMenu, UI-примитивы (Button/Input/Card/Logo/...), `EmblaCarousel`+`CarouselDots`+`CarouselArrows` из Sprint 1.

(`CarouselArrows` уже корректно скрыты на мобайле через `hidden md:flex` — проверено в исходниках Sprint 1, §T3 «стрелки только ≥ md» выполнен.)
...
1. Новые зависимости и UI-примитивы:
   - npm i @radix-ui/react-checkbox
   - components/ui/Badge.tsx — минимальный примитив для бейджей карточек
   - components/ui/Checkbox.tsx — обёртка над Radix Checkbox
...
**Создаём:**
- `content/home.ts`
- `components/sections/{Hero,HeroBackground,About,WorkAreas,WorkAreasMobileCarousel,WorkAreaCard,Cases,CasesMobileCarousel,CaseCard,Services,ServicesCarousel,ServiceCard,ContactCtaBanner,FAQ,FAQAccordion,ContactForm}.tsx`
- `components/ui/Badge.tsx`, `components/ui/Checkbox.tsx`
- `components/icons/WorkAreaIcons.tsx`
- `public/images/hero.webp`, `public/images/about.webp`

**Новые зависимости:**
- `@radix-ui/react-checkbox` (для UI-примитива `Checkbox`)
